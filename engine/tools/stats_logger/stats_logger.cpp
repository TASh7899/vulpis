...
namespace Vulpis {
  namespace Tools {
    StatsLogger::StatsLogger(const std::string& filename) {
      file.open(filename);
      if (file.is_open()) {
        file << "Time(ms),DeltaTime(s),FPS,RAM(MB),LayoutTime(ms),RenderTime(ms)\n";
      }
      buffer.reserve(1000);
    }
    ...
    void StatsLogger::log(uint32_t time, float dt, double layoutMs, double renderMs) {
      double currentRam = GetCurrentRAMUsageMB();
      buffer.push_back({time, dt, currentRam, layoutMs, renderMs});
      if (buffer.size() >= 1000) {
        flush();
      }
    }
